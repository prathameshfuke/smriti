...
export const SunfishSVG = memo(({
    ...
    if (isGlowing && !isChecking) {
        // Initially glowing to memorize. White reads as barely-different against
        // the pale teal play-area background — a dementia patient needs a color
        // that stays visible against a light backdrop, so this uses a saturated
        // magenta with a wide, layered, pulsing glow instead.
        strokeColor = '#FF2D95';
        strokeWidth = "6";
        filter =
            'drop-shadow(0 0 10px #FF2D95) drop-shadow(0 0 22px rgba(255,45,149,0.85)) drop-shadow(0 0 36px rgba(255,45,149,0.6))';
    } else if (isChecking) {
    ...
    return (
        <div
            ...
            className={`relative flex items-center justify-center will-change-transform ${isSelected ? 'fish-bounce' : ''} ${isGlowing && !isChecking ? 'fish-glow-pulse' : ''}`}
        >
            ...
            <style jsx>{`
                ...
                @media (prefers-reduced-motion: no-preference) {
                    .fish-glow-pulse { animation: glow-pulse 0.7s infinite ease-in-out; }
                }
            `}</style>
